...
import { Home, Building2, TrendingUp, LogOut, Plus, Edit, Trash2, Settings, Eye, Clock, Monitor, Smartphone, Tablet } from "lucide-react";
...
export default function BuilderDashboard() {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        // Verify user is still a Builder
        ...
        fetchProperties(data.data.email);
        fetchAnalytics(data.data.email);
      }
    } catch (error) {
      router.push('/login');
    }
  ...
  const fetchAnalytics = async (email: string) => {
    try {
      const response = await fetch(`/api/analytics?type=builder&email=${email}&days=30`);
      const result = await response.json();
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card border-b border-border sticky top-0 z-50">
        <div className="container mx-auto px-4 lg:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-heading font-bold text-foreground">Builder Dashboard</h1>
              <p className="text-xs text-muted-foreground">Welcome, {userData?.name} - Role: Builder</p>
            </div>
          </div>
      ...
      <main className="container mx-auto px-4 lg:px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <Link href="/admin/dashboard/analytics?mode=builder">
            <div className="bg-card rounded-xl p-6 border border-border card-shadow hover:card-shadow-hover transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">View your property insights</p>
                </div>
              </div>
            </div>
          </Link>
          <Link href="/admin/dashboard/add-property?mode=builder">
            <div className="bg-card rounded-xl p-6 border border-border card-shadow hover:card-shadow-hover transition-all cursor-pointer group">
              <div className="flex items-center gap-4">
        ...
        {!analyticsLoading && analytics && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            {/* Top Properties */}
            <div className="bg-card rounded-xl border border-border card-shadow p-6">
              <h3 className="text-lg font-heading font-bold text-foreground mb-4">Top Viewed Properties</h3>
              <div className="space-y-3">
                {analytics.topProperties?.slice(0, 5).map((property: any, index: number) => (
                  <div key={property._id} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                      <span className="text-sm font-bold text-primary">#{index + 1}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-foreground truncate">{property.title || 'Untitled'}</p>
        ...
        <div className="bg-card rounded-xl border border-border card-shadow">
          <div className="p-6 border-b border-border flex items-center justify-between">
            <div>
              <h2 className="text-xl font-heading font-bold text-foreground">My Properties</h2>
              <p className="text-sm text-muted-foreground mt-1">Manage all your property listings</p>
            </div>
            <Link href="/admin/dashboard/add-property?mode=builder">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-muted-foreground">No properties found. Add your first property!</div>
          ) : (
                  ...
                  {properties.map((property) => {
                    ...
                    return (
                      <tr key={property._id} className="hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="px-6 py-4 text-sm font-semibold text-foreground">{property.price}</td>
                        <td className="px-6 py-4">
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {isApproved && (
                              <Button 
                                ...
                                onClick={() => router.push(`/admin/dashboard/edit-property/${property._id}?mode=builder`)}
                              >
